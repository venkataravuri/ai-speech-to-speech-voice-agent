...
class LLMModule:
    ...
    def load_model(self):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(self.model_name, low_cpu_mem_usage=True,  offload_state_dict = True).cpu()
        disk_offload(model=self.model, offload_dir="alpha")
        self.pipeline = pipeline("text-generation", model=self.model, tokenizer=self.tokenizer)
        self.max_context = self.max_context
        self.history = []
...
# Singleton instance of the LLM Module
